...
# AZ gene burden only
PYTHONPATH=$(pwd) time python modules/AzGeneBurden.py \
    ...
    --output azphewas_gene_burden.tsv

# GeneBass gene burden only
PYTHONPATH=$(pwd) time python modules/GenebassGeneBurden.py \
    --genebass_data "$DATA_DIR/genebass/" \
    --output genebass_gene_burden.tsv

# Curated gene burden only
PYTHONPATH=$(pwd) time python modules/CuratedGeneBurden.py \
    --curated_data "$DATA_DIR/curated_evidence.tsv" \
    --output curated_evidence_output.tsv

# FinnGen gene burden only
PYTHONPATH=$(pwd) time python modules/FinngenGeneBurden.py \
    --finngen_data "$DATA_DIR/finngen/finngen_R11_lof.txt" \
    --finngen_manifest "$DATA_DIR/finngen/finngen_manifest.json" \
    --output finngen_gene_burden.tsv
